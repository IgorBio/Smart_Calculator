...
TEST(MathCalcTest, Exception) {
  EXPECT_THROW(MathCalc::Calculate(""), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate(" "), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("5*a"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("5!"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("5 7"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("(+-/)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("5**7"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("**"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("3*"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("/7"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("."), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("()"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("sinb(x)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("tan(2q)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("sqrt(r)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("5fmod4"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("x 55"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("2 + 5.5.5"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("2mod"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("2asin"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("2*s in(3)"), std::logic_error);

  EXPECT_THROW(MathCalc::Calculate("mod3"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("3mob2"), std::logic_error);

  EXPECT_THROW(MathCalc::Calculate("e"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("e-1"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("1.2e"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("1.2e+3.4"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("cos(12)e-3"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("1.2e*cos(34)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("1.2ee-3"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate(".e1"), std::logic_error);

  EXPECT_THROW(MathCalc::Calculate("(x))"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("((x)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("log(x"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("cosx"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("sin)(x)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("tan(x))"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("asin(x("), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("acos)x)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("atan((x)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("sqrtx"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("2+3)ln)/4"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("8 *)*cos(25)"), std::logic_error);
  EXPECT_THROW(MathCalc::Calculate("pow(3, 2)"), std::logic_error);
